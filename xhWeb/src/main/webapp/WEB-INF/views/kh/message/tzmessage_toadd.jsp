								...
								<div class="ibox ">
									<div class="ibox-content">
										<div>
											...
											<div class="row">
												<div style="margin-left: 6px" class="col-md-3">
													<input type="checkbox" id="cb_znfb" /> <span
														style="margin-left: 6px; font-size: 12px">发布为站内消息</span>
												</div>
											</div>
											<div class="row">
												<div style="margin-left: 6px" class="col-md-3">
													<input type="checkbox" id="cb_dxfb" /> <span
														style="margin-left: 6px; font-size: 12px">发布为短信消息</span>
												</div>
											</div>
...
<rapid:override name="script">
	...
	<script>
		...
		function showAdd() {
			...
			if ($(obj).is(":visible")) {//如何是显示就隐藏
				$(obj).hide("slow", function() {
					$("#btn_add").html('<i class="fa fa-plus"></i>&nbsp;添加接收人');
				});
				...
				shade : false
					...
					function() {
						...
						$("#to option").each(function() {
									var indexValue = $(this).attr("index");//取到的是userId
									if (indexValue == userId) {
										$(this).clone().appendTo("#from");
										$("#to option[index=" + indexValue + "]").remove();//移除一个用户
									}
								});
						layer.msg("正在更新...");
						//重新加载选中的用户或已经删除掉的用户
		...
		var initjQgrid = function() {
			...
			if (resultData == null || resultData.length == 0) {
				...
				if(resultData.length>2){//按照部门名称排序，将部门的人按顺序放在一起
				...
				//显示全部删除按钮 如何选中的联系人列表长度大于0则显示删除按钮
	...
	$("#btn_send").click(function() {
					...
					if ($("#cb_znfb").is(":checked")) {
						znFB = true;
					}
					if ($("#cb_dxfb").is(":checked")) {
						dxFB = true;
					}
					...
					function(data,ss) {
						...
						if (data.result == "success") {
							layer.msg("发送成功");
							window.location.href = "${ctx}/message/getZnmessage/";
						} else {
